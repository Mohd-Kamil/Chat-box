...
async function generateChatResponse(prompt: string, context: any = {}): Promise<string> {
  ...
  try {
    ...
    const systemPrompt = `You are ShawnGPT, a friendly and conversational AI assistant. You should respond in a natural, engaging way with some Hinglish (Hindi-English mix) and Indian street slang when appropriate. Keep responses conversational, helpful, and not too formal.`;
    ...
    if (data && data.candidates && data.candidates[0] && data.candidates[0].content) {
      ...
      // Clean up the response - remove any "ShawnGPT:" prefix
      aiResponse = aiResponse.replace(/^ShawnGPT:\s*/i, "").trim();
      
      // If we got a meaningful response, use it
      if (aiResponse.length > 5 && aiResponse.length < 500) {
        console.log("Using Gemini API response:", aiResponse);
        return aiResponse;
      }
    }
