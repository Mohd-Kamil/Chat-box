...
// Chat Mode - Using DeepSeek API for immersive, ChatGPT-like conversations
async function generateChatResponse(prompt: string, context: any = {}): Promise<string> {
  const apiKey = process.env.DEEPSEEK_API_KEY;
  if (!apiKey) {
    console.log("DeepSeek API key not found, using enhanced fallback");
...
async function generateDetailedResponse(prompt: string, context: any = {}, mode: string = 'chat'): Promise<string> {
  // Always use DeepSeek to summarize/fuse all API results into a conversational answer
...
// Update analyzeIntentWithGemini to DeepSeek
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/chats/:id/messages", async (req, res) => {
    try {
      ...
      let deepseekIntent: any = null;

      // If mode is 'auto' or not provided, use DeepSeek for intent detection
      if (!mode || mode === 'auto') {
        deepseekIntent = await analyzeIntentWithGemini(messageData.content, recentMessages);
        mode = deepseekIntent.mode;
        // Use DeepSeek's suggested APIs for fusion
        detectedModes = [];
        if (deepseekIntent.apis?.includes('TMDB')) detectedModes.push('cinephile');
        if (deepseekIntent.apis?.includes('RAWG')) detectedModes.push('game');
        if (deepseekIntent.apis?.includes('Serper')) detectedModes.push('research');
        if (detectedModes.length === 0) detectedModes.push(mode as ChatMode);
        // --- Update currentTopic based on DeepSeek's detected entities ---
        // Prefer movie, game, or person entity, fallback to previous topic
        let newTopic = null;
        if (deepseekIntent.entities) {
          if (deepseekIntent.entities.movie) newTopic = deepseekIntent.entities.movie;
          else if (deepseekIntent.entities.game) newTopic = deepseekIntent.entities.game;
          else if (deepseekIntent.entities.person) newTopic = deepseekIntent.entities.person;
          else if (deepseekIntent.entities.topic) newTopic = deepseekIntent.entities.topic;
        }
      ...
      // Always use DeepSeek to summarize/merge all API results into a conversational answer, with session memory
