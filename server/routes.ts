...
async function generateChatResponse(prompt: string, context: any = {}): Promise<string> {
  ...
  if (!apiKey) {
    console.log("HF API key not found, using enhanced fallback");
    return generateEnhancedChatResponse(prompt);
  }

  try {
    console.log("Attempting HF API call for chat mode");
    
    // Try a more conversational model
    ...
    const conversationPrompt = `User: ${prompt}\nAssistant:`;
    ...
    const data = await response.json();
    console.log("HF API response received:", data);
    
    if (data && Array.isArray(data) && data[0] && data[0].generated_text) {
      const generatedText = data[0].generated_text;
      let aiResponse = generatedText;
      
      // Clean up the response
      if (aiResponse.includes(conversationPrompt)) {
      ...
      if (aiResponse.length > 5 && aiResponse.length < 400) {
        console.log("Using HF API response:", aiResponse);
        return aiResponse;
      }
    }
    
    // If HF API didn't give a good response, use enhanced fallback
    console.log("HF API response not suitable, using enhanced fallback");
    return generateEnhancedChatResponse(prompt);
