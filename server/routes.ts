...
// Chat Mode - Using Google Gemini API for real AI responses
async function generateChatResponse(prompt: string, context: any = {}): Promise<string> {
  ...
  try {
    console.log("Attempting Gemini API call for chat mode");
    
    // Use the correct Google AI API endpoint
    const modelUrl = "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent";
    
    // Create a conversational prompt with Hinglish personality
    const conversationPrompt = `You are a friendly AI assistant who speaks in Hinglish (Hindi + English) with Indian street slang. Be conversational, casual, and use words like "yaar", "bro", "dude", "bhai", etc. Keep responses short and engaging (50-150 words). User says: ${prompt}`;
    ...
    if (data && data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0]) {
      ...
      if (generatedText && generatedText.length > 5 && generatedText.length < 300) {
        console.log("Using Gemini API response:", generatedText);
        return generatedText;
      }
    }
    
    // If Gemini API didn't give a good response, use enhanced fallback
    console.log("Gemini API response not suitable, using enhanced fallback");
    return generateEnhancedChatResponse(prompt);
...
function generateEnhancedChatResponse(prompt: string): string {
  ...
  // Food and hunger related queries
  if (lowerPrompt.includes('hungry') || lowerPrompt.includes('eat') || lowerPrompt.includes('food') || lowerPrompt.includes('meal')) {
    const foodResponses = [
      "Arre yaar, hunger is real! 😋 What are you in the mood for? Quick snacks like samosa, pakora, or something more filling like biryani, pizza, or maybe some healthy options? Tell me your preference and I'll give you some tasty suggestions!",
      "Hungry? Let's fix that! 🍕 What's your vibe today - Indian food like butter chicken and naan, or maybe some international cuisine? Or if you want something quick, how about a sandwich or some street food?",
      "I can totally relate to that hunger! 🍔 What sounds good to you? We've got options like pasta, Chinese food, or maybe some comfort food like dal-rice? What's your favorite type of cuisine?"
    ];
    return foodResponses[Math.floor(Math.random() * foodResponses.length)];
  }
  
  // Weather related queries
  if (lowerPrompt.includes('weather') || lowerPrompt.includes('hot') || lowerPrompt.includes('cold') || lowerPrompt.includes('rain')) {
    return "Weather talk! 🌤️ I can't check the actual weather right now, but I can chat about it! What's the weather like where you are? Hot, cold, rainy, or perfect?";
  }
  
  // Work and study related
  if (lowerPrompt.includes('work') || lowerPrompt.includes('study') || lowerPrompt.includes('job') || lowerPrompt.includes('office')) {
    const workResponses = [
      "Work life can be challenging yaar! 💼 How's your day going? Need a break or some motivation? I'm here to chat about whatever's on your mind!",
      "Ah, the daily grind! 📚 Work and studies can be tough sometimes. What's keeping you busy? Need to vent or just want some company?",
      "Work mode activated! 💪 How's it going? Sometimes we all need someone to talk to during the workday. What's happening?"
    ];
    return workResponses[Math.floor(Math.random() * workResponses.length)];
  }
  
  // Technology and gadgets
  if (lowerPrompt.includes('phone') || lowerPrompt.includes('computer') || lowerPrompt.includes('tech') || lowerPrompt.includes('gadget')) {
    return "Tech talk! 📱 I love discussing technology! What gadget are you thinking about? Phones, laptops, or something else? I'm all ears for tech conversations!";
  }
  
  // Music and entertainment
  if (lowerPrompt.includes('music') || lowerPrompt.includes('song') || lowerPrompt.includes('movie') || lowerPrompt.includes('watch')) {
    return "Entertainment time! 🎵 What are you into these days? Music, movies, or maybe some TV shows? I'd love to hear your recommendations!";
  }
  
  // Travel and places
  if (lowerPrompt.includes('travel') || lowerPrompt.includes('place') || lowerPrompt.includes('visit') || lowerPrompt.includes('trip')) {
    return "Travel dreams! ✈️ Where would you love to go? Any places on your bucket list? I'm always curious about people's travel plans and experiences!";
  }
