...
- Keep responses conversational and engaging (100-300 words)
- Don't be robotic or formal - be warm and human-like

**Current User Message:** ${prompt}

**Your Response:** (Respond naturally as ShawnGPT, being immersive and engaging)`;
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/chats/:id/messages", async (req, res) => {
    try {
      ...
      // If mode not provided, use intent classifier
      if (!mode) {
        mode = detectMode(messageData.content);
        intent = detectIntent(messageData.content);
        // Multi-API fusion: if multiple intents detected, add all relevant modes
        if (intent.movies) detectedModes.push('cinephile');
        if (intent.games) detectedModes.push('game');
        if (intent.search) detectedModes.push('research');
        if (detectedModes.length === 0) detectedModes.push(mode as ChatMode);
      } else {
        detectedModes = [mode as ChatMode];
      }

      // Store mode history in session (if available)
      // (Commented out: req.session is not guaranteed to exist)
      // if (!req.session) req.session = {};
      // req.session.lastModes = detectedModes;
      ...
      // Always use Gemini to summarize/merge all API results into a conversational answer
      const aiResponse = await withRetry(() => generateDetailedResponse(messageData.content, context, detectedModes[0]));
      ...
      const response: ChatResponse = {
        ...
        movies: context.movies,
        games: context.games,
        // people: context.people, // Not in ChatResponse type
        // modes: detectedModes,   // Not in ChatResponse type
      };
      res.json(response);
    } catch (error) {
