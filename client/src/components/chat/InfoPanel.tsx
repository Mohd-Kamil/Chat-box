...
export default function InfoPanel({ selectedMode, onModeChange }: { selectedMode: string, onModeChange: (mode: string) => void }) {
  ...
  return (
    <div className="w-full bg-card-dark border-l border-border-gray flex flex-col h-full pointer-events-auto relative z-10">
      {/* Panel Header */}
      <div className="p-6 border-b border-border-gray flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {MODES.map((mode) => (
              <button
                key={mode.key}
                className={`px-3 py-1 rounded-full text-xs font-medium flex items-center border transition-colors ${selectedMode === mode.key ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-800 text-gray-300 border-slate-700 hover:bg-blue-700 hover:text-white'}`}
        ...
        <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-600/30 rounded-lg p-4">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <h4 className="text-white font-medium">Smart Mode Detection</h4>
          </div>
          <p className="text-sm text-gray-300 mb-3">
            You can now select a mode above! Each mode uses a different API and personality:
          </p>
      ...
      <div className="p-6 border-t border-border-gray flex-shrink-0">
