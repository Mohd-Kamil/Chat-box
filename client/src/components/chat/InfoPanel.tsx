    ...
    <div className="w-full bg-slate-900/95 backdrop-blur-xl border-l border-slate-700 flex flex-col h-full relative z-10">
      ...
      <div className="flex-1 overflow-y-auto p-3 lg:p-6 space-y-3 lg:space-y-4">
        ...
        <div className="mb-2 lg:mb-4">
          <label className="block text-xs text-gray-400 mb-1 lg:mb-2">Select Mode:</label>
          <div className="flex gap-1 lg:gap-2 flex-wrap">
            {MODES.map((mode) => (
              <button
                key={mode.key}
                className={`px-2 lg:px-3 py-1 rounded-full text-xs font-medium flex items-center border transition-colors ${selectedMode === mode.key ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-800 text-gray-300 border-slate-700 hover:bg-blue-700 hover:text-white'}`}
