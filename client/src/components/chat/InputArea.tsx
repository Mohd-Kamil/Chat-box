    ...
    <div className="border-t border-slate-700 bg-slate-900/90 backdrop-blur-xl relative z-10">
      <div className="max-w-4xl mx-auto px-4 py-4 lg:py-6">
        <div className="relative">
          <div className="flex items-end space-x-3">
            <div className="flex-1 relative">
              <Textarea
                ...
                className={cn(
                  "w-full bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl px-4 lg:px-6 py-3 lg:py-4 pr-24 lg:pr-32 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none min-h-[48px] lg:min-h-[56px] max-h-32",
              ...
              <Button
                ...
                className={cn(
                  "absolute right-12 lg:right-16 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-indigo-400 transition-colors p-1 lg:p-2",
          ...
          <div className="flex flex-wrap gap-2 mt-3 lg:mt-4">
            {quickActions.map((action) => (
              <Button
                ...
                className="px-3 lg:px-4 py-1.5 lg:py-2 bg-slate-800/50 hover:bg-slate-700/70 text-gray-300 rounded-full text-xs lg:text-sm transition-colors backdrop-blur-sm border border-slate-700"
                disabled={disabled}
              >
                {action.label}
              </Button>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mt-2 lg:mt-3 text-xs text-gray-500 space-y-1 lg:space-y-0">
            <span>Press Enter to send, Shift+Enter for new line</span>
            <span>Powered by Gemini AI, TMDb, RAWG & Serper</span>
          </div>
        </div>
      </div>
    </div>
