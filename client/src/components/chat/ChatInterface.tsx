...
export default function ChatInterface({ initialChatId }: ChatInterfaceProps) {
  ...
  const [pendingBotMessage, setPendingBotMessage] = useState<null | { id: number; content: string; role: string; createdAt: Date; chatId: number; metadata: any }>(null);

  // Always sync displayedMessages with real messages
  useEffect(() => {
            ...
            {displayedMessages.length === 0 ? (
              ...
              <div className="space-y-3 lg:space-y-6">
                {displayedMessages.map((msg, i) => (
                  <MessageCard key={msg.id} message={msg} isLatest={i === displayedMessages.length - 1 && !pendingBotMessage} />
                ))}
                {pendingBotMessage && (
                  <MessageCard key={pendingBotMessage.id} message={pendingBotMessage} isLatest />
                )}
                {localIsLoading && (
