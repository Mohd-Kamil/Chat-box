...
export default function ChatInterface({ initialChatId }: ChatInterfaceProps) {
  ...
  // Sync displayedMessages with real messages when not loading
  useEffect(() => {
    if (!localIsLoading) setDisplayedMessages(messages);
  ...
  const handleSendMessage = async (content: string) => {
    ...
    // Remove temp user message if replaced by real one
    setTimeout(() => {
      setDisplayedMessages(msgs => msgs.filter(m => m.id !== tempId));
    }, 1000);
  };
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        <div className="w-full bg-slate-800/70 border-b border-slate-700 px-2 py-2 flex items-center gap-2">
          <span className="text-xs text-gray-400 mr-2">Mode:</span>
          <select
            value={selectedMode}
            onChange={e => setSelectedMode(e.target.value)}
            className="bg-slate-900 text-white text-xs rounded px-2 py-1 border border-slate-700 focus:outline-none"
          >
            {modeOptions.map(opt => (
              ...
              <div className="space-y-3 lg:space-y-6">
                {displayedMessages.map((message) => (
                  <MessageCard
                    key={message.id}
                    message={message}
                    isLatest={displayedMessages[displayedMessages.length - 1]?.id === message.id}
                  />
                ))}
                {localIsLoading && (
