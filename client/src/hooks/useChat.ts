...
export function useChat(initialChatId?: number) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async ({ content, chatId, mode }: { content: string; chatId: number; mode?: string }) => {
      try {
  ...
  const sendMessage = async (content: string, chatId?: number, mode?: string) => {
    const targetChatId = chatId || currentChatId;
    if (!targetChatId) {
      throw new Error('No chat selected');
    }

    return sendMessageMutation.mutateAsync({ content, chatId: targetChatId, mode });
  };
